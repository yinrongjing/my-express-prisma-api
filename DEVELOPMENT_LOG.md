...
- [ ] `prisma.config.ts` 的 url 有兜底值？（`\|\| "file:/data/dev.db"`）
- [ ] `src/lib/prisma.ts` 的 url 用了 `process.env.DATABASE_URL`？（不是硬编码 `./dev.db`）
- [ ] 搜一下还有没有漏网之鱼：`grep -rn "dev\.db" src/ prisma/ --include='*.ts' \| grep -v "process.env"`
- [ ] Dockerfile CMD 先 migrate 再 start？（`prisma migrate deploy && npm start`）
- [ ] `cors` 中间件在路由**前面**？（`app.use(cors())` 在 `app.use('/api/todos')` 之前）
- [ ] 所有依赖都在 package.json 里？（`npm install cors`、`npm install -D @types/cors`）
- [ ] Railway 环境变量：`DATABASE_URL = file:/data/dev.db`
- [ ] Railway Volume：挂载 `/data`，至少 1GB

### 前端（Vercel）

- [ ] Vercel 环境变量：`VITE_API_BASE = https://后端地址/api`
- [ ] 项目根目录有 `vercel.json`，配置了 SPA rewrite
- [ ] `src/utils/request.ts` 的 baseURL 用了 `import.meta.env.VITE_API_BASE`

### 一键检查脚本

```bash
# 在后端项目根目录跑，有输出 = 有问题
grep "app.use(cors" src/index.ts && echo "✅ cors 已引入" || echo "❌ 缺 cors"
grep "process.env" src/lib/prisma.ts && echo "✅ prisma 用环境变量" || echo "❌ prisma 硬编码"
grep "process.env" prisma.config.ts && echo "✅ config 用环境变量" || echo "❌ config 硬编码"
grep "migrate deploy" Dockerfile && echo "✅ 启动前迁移" || echo "❌ 缺迁移"
```
